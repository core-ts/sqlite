...
var PoolManager = (function () {
  ...
  PoolManager.prototype.executeScalar = function (sql, args) {
    return executeScalar(this.db, sql, args);
  };
...
function execBatch(db, statements) {
  return execute(db, 'begin transaction').then(function () {
    return new Promise(function (resolve, reject) {
      var c = 0;
      statements.forEach(function (item, index) {
        db.run(item.query, toArray(item.args), function (err, result) {
          if (err) {
            reject(err);
          }
          else {
...
function executeScalar(db, sql, args) {
...
function count(db, sql, args) {
  return executeScalar(db, sql, args);
}
exports.count = count;
